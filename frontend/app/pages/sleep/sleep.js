document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function loadChildren() {
        try {
            const response = await fetch('/WEB_project/backend/api/children.php?action=list', {
                method: 'GET',
            ...
            if (result.status === 'success' && result.children.length > 0) {
                ...
                sleepChildSelect.addEventListener('change', (e) => {
                    ...
                    loadSleepDataForChild(e.target.value);
                });

                updateChildAgeDisplay(result.children[0].birth_date);
                loadSleepDataForChild(result.children[0].id);
    ...
    sleepForm.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetch('/WEB_project/backend/api/sleep.php?action=add', { 
                method: 'POST', 
            ...
            if (result.status === 'success') {
                closeModal();
                loadSleepDataForChild(sleepChildSelect.value); 
            } else {
    ...
    async function loadSleepDataForChild(childId) {
        ...
        try {
            const response = await fetch(`/WEB_project/backend/api/sleep.php?action=getMetrics&child_id=${childId}`);
            const result = await response.json();
            ...
            if (result.status === 'success') {
                ...
                if (sleepLogs.length === 0) {
                    ...
                    top5Logs.forEach(log => {
                        const notaText = log.notes ? log.notes : `Înregistrare adăugată.`;
                ...
                if (document.getElementById('statTotalAzi')) document.getElementById('statTotalAzi').textContent = doarSomnuri.length + " ses.";
                if (document.getElementById('statNoapteAzi')) document.getElementById('statNoapteAzi').textContent = formatTime(totalNoapteMins);
                if (document.getElementById('statZiAzi')) document.getElementById('statZiAzi').textContent = formatTime(totalZiMins);
                ...
                if (statCalitateText && statCalitateStele) {
                    if (cateCalitati > 0) {
                        let media = Math.round(sumaCalitate / cateCalitati);
                ...
                if(btnVeziNotite) {
                    btnVeziNotite.onclick = (e) => {
                        ...
                        modalContainer.innerHTML = ''; // Golim
                        sleepLogs.forEach(log => {
